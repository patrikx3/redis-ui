...
* REFACTOR: Improved the Vue dialog components for a smoother UI experience.
...
* FEATURE: Added AI-powered command hints for Redis commands.
* BUGFIX: Fixed the tree node expand/collapse toggle behavior.
* CHORE: Upgraded Angular and updated dependencies to the latest versions.

### v2026.4.3012
Released on 06/22/2026
* FEATURE: Added Meet Assistant SaaS promo alongside the AI Network Assistant on the demo site.
* BUGFIX: Fixed a duplicate KeyBinding type error by upgrading the CodeMirror editor.
* DOCS: Added features documentation and updated the comparison page.
* DOCS: Updated repository URLs to the new domain.
* CHORE: Updated dependencies to the latest versions.
...
* FEATURE: Added array data type support for Redis keys.
* FEATURE: Added AI-powered command hints and array operations on the server.
* DOCS: Updated comparison and added features documentation.
* CHORE: Upgraded Angular and updated dependencies across redis-ui, redis-ui-material, and redis-ui-server.

### v2026.4.3001
Released on 05/21/2026
* FEATURE: Added Meet Assistant SaaS promo alongside the AI Network Assistant on the demo site.
* BUGFIX: Corrected tree node expand/collapse toggle behavior.
* BUGFIX: Resolved duplicate KeyBinding type error by pinning @codemirror/view to 6.42.1.
* CHORE: Updated dependencies to latest versions across redis-ui, redis-ui-material, and redis-ui-server.

### v2026.4.2012
Released on 05/08/2026
* FEATURE: macOS builds now ship both arm64 and x64 architectures (20 release assets, up from 16).
* DOCS: Expanded documentation with detailed artifact guides.
...
* DOCS: Updated documentation across Material UI package.

### v2026.4.2005
Released on 04/19/2026
* FEATURE: Vue added to Electron menu with simplified main menu structure.
* BUGFIX: Fixed auto-language crash and Content Security Policy issue in Electron.
* BUGFIX: Updated Angular build configuration for compatibility.
* BUGFIX: Hidden native scrollbar on database tree with tightened overlay thumb inset.
* BUGFIX: Server now uses BGSAVE instead of SAVE for non-blocking Redis snapshots.
* PERF: Hoisted TTL computation out of per-row render loop in database tree.
* REFACTOR: Removed dead /console route from Vue.
...
* REFACTOR: Simplified main menu structure.
* BUGFIX: Fixed auto-language crash and CSP issues.
* CHORE: Updated dependencies to latest versions.
...
* BUGFIX: Vue scrollbar styling now respects the active theme.

### v2026.4.1003
Released on 04/13/2026
* FEATURE: ACL management UI redesign with chip-based editor, dialog popups, auto-load, and readonly guards.
* FEATURE: ACL management cluster broadcast, server-side validation, and new handlers.
* BUGFIX: ACL chip styling now uses theme-controlled colors instead of hardcoded values.
* BUGFIX: Vue tree TTL icon vertical centering with inline-flex on node wrapper.
...
* REFACTOR: Updated socket.io action names to area/function format.
* BUGFIX: Fixed React Groq validation.

### v2026.4.806
Released on 04/10/2026
* BUGFIX: Applied formatValue before truncateDisplay to ensure correct value display.
...
### v2026.4.802
Released on 04/10/2026
* FEATURE: Added diff view before save with settings toggle for comparing changes.
* FEATURE: Added undo for string and JSON key edits with settings toggle.
* FEATURE: Added dashboard info cards to the Pulse page.
* FEATURE: Added panel split persistence across sessions.
* BUGFIX: Fixed diff view to coerce values to string for binary key support.
* BUGFIX: Fixed dialog Escape key handling.
* BUGFIX: Fixed settings page styling.
* BUGFIX: Fixed React accordion uPlot resize on Pulse page.
* DOCS: Added diff view, undo, and dashboard info cards documentation to README.
...
### v2026.4.639
Released on 04/06/2026
* FEATURE: Added server-side AI command execution with bash pipe support for enhanced Redis CLI integration.
...
* FEATURE: Added AI error code translations for all 53 languages (blocked_api_access, rate_limit, AI_DISABLED, etc.).
* BUGFIX: Normalized Groq API error codes (blocked_api_access, rate_limit).
* BUGFIX: Fixed version overlay color inheritance in React Electron.

### v2026.4.631
Released on 04/05/2026
* FEATURE: Added VS Code extension for P3X Redis UI.
* BUGFIX: Show version overlay in React Electron.
* BUGFIX: Refresh tree after TTL change.

### v2026.4.630
Released on 04/05/2026
* FEATURE: Run E2E tests before publish to catch errors early.
* FEATURE: Support `P3XR_E2E_EXTRA_ARGS` environment variable for extra Playwright options in E2E testing.

### v2026.4.629
Released on 04/05/2026
* BUGFIX: Resolved TypeScript AOT build error in the settings service.
* BUGFIX: Removed aggressive database redirect behavior.
...
### v2026.4.623
Released on 04/04/2026
* CHORE: Updated the license field to MIT.

### v2026.4.622
Released on 04/04/2026
* REFACTOR: Simplified publish workflow by removing Grunt version bump steps from submodule repositories.


### v2026.4.618
Released on 04/04/2026
* FEATURE: Show version in the header on all platforms including Electron.
* BUGFIX: Resolve version detection by scanning sibling directories and walking the directory tree for flat-hoisted dependencies.
* REFACTOR: Extract version resolution into a shared module.

### v2026.4.616
Released on 04/04/2026
* REFACTOR: Simplify screenshot capture script and update screenshots.

### v2026.4.614
Released on 04/04/2026
* REFACTOR: Use live connection via UI instead of local config file.
* CHORE: Consolidate preview images into single optimized image.

### v2026.4.612
Released on 04/04/2026
* BUGFIX: Use package version directly instead of config for the health endpoint.
* REFACTOR: Simplify Cache-Control headers for HTML responses.
...
[**P3X-REDIS-UI**](https://corifeus.com/redis-ui) Build v2026.10.100
